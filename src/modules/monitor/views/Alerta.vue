  ...
  <div>
    ...
    <b-table :sort-by.sync="sortBy" striped hover :items="items" :fields="fields" class="text-center" responsive="sm">
      ...
      <template v-slot:cell(mandar_alerta)="data">
        <b-button variant="outline-danger" size="sm" :to="`./Enviar/${data.item.id_orden}`">Enviar <i class="fas fa-exclamation-triangle"></i></b-button>
      </template>
    </b-table>
      ...
      this.axios.get('/ordenes', config)
        .then((res) => {
          for (let i = 0; i < res.data.length; i++) {
            if(res.data[i].resultado == "POSITIVO" && res.data[i].alerta_enviada == false) {
              this.items.push({
