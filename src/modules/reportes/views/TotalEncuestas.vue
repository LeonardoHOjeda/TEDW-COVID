...
<script>
  ...
  export default {
    ...
    async created() {
      await this.fetchTotalEncuestas();
      this.totalEncuestas.carrera.forEach((item) => {
        const color = this.generateColor();
        this.chartDataCarreras.labels.push(item.carrera);
        this.chartDataCarreras.datasets[0].data.push(item.total_encuestas);
      ...
      this.totalEncuestas.departamento.forEach((item) => {
        const color = this.generateColor();
        this.chartDataDepartamentos.labels.push(item.departamento);
        this.chartDataDepartamentos.datasets[0].data.push(item.total_encuestas);
